...
var _ = Describe("Version", func() {
	...
	Context("matchesRegex", func() {
		It("works", func() {
			...
			Expect(versionutils.MatchesRegex("v1.2.3-beta1-wasm")).To(BeFalse(), "should reject non-suffix numbers in the label")
			Expect(versionutils.MatchesRegex("v1.2.3-beta10-wasm")).To(BeFalse(), "should reject non-suffix numbers in the label")
			Expect(versionutils.MatchesRegex("v1.0.0-rc-wasm")).To(BeTrue(), "should allow hyphenated tags")
			...
			Expect(versionutils.MatchesRegex("v1.2.3-foo-bar")).To(BeTrue(), "should allow hyphenated-label")
			Expect(versionutils.MatchesRegex("v1.2.3-foo1-bar")).To(BeFalse(), "should reject version in middle of label")
			Expect(versionutils.MatchesRegex("v1.2.3-foo1-bar1")).To(BeFalse(), "should reject multiple versions")
			Expect(versionutils.MatchesRegex("v1.2.3-foo-bar1")).To(BeTrue(), "should allow trailing label version")
			Expect(versionutils.MatchesRegex("v1.2.3-foo-bar10")).To(BeTrue(), "should allow multi-digit trailing label version")
			Expect(versionutils.MatchesRegex("v1.2.3-foo-bar-baz-bats")).To(BeTrue(), "should allow multi-hyphen label")
			Expect(versionutils.MatchesRegex("v1.2.3-foo-bar-baz-bats1")).To(BeTrue(), "should allow multi-hyphen label with version")
			Expect(versionutils.MatchesRegex("v1.2.3-foo1-bar-baz-bats1")).To(BeFalse(), "should reject multi-hyphen multi-version label")
			Expect(versionutils.MatchesRegex("v1.2.3-foo-bar2-baz-bats1")).To(BeFalse(), "should reject multi-hyphen multi-version label")
			Expect(versionutils.MatchesRegex("v1.2.3-foo-bar-baz3-bats")).To(BeFalse(), "should reject multi-hyphen multi-version label")
		})
	})
	...
	Context("ParseVersion", func() {
		...
		It("supports wasm versions", func() {
			parsed, err := versionutils.ParseVersion("v1.5.0-wasm-rc1")
			Expect(err).NotTo(HaveOccurred())
			Expect(parsed.Label).To(Equal("wasm-rc"))
			Expect(parsed.LabelVersion).To(Equal(1))

			parsed, err = versionutils.ParseVersion("v1.5.0-wasm-beta2")
			Expect(err).NotTo(HaveOccurred())
			Expect(parsed.Label).To(Equal("wasm-beta"))
	...
	Context("IsGreaterThanTag", func() {
		...
		It("works", func() {
			...
			expectResult("v1.0.0-rc1", "v1.0.0-rc2", false, true, "")
			expectResult("v1.0.0-rc2", "v1.0.0-rc1", true, true, "")
			expectResult("v1.0.0-wasm-rc1", "v1.0.0-wasm-rc2", false, true, "")
			expectResult("v1.0.0-wasm-rc2", "v1.0.0-wasm-rc1", true, true, "")
			expectResult("v1.0.0-wasm", "v1.0.0", false, false, "")
			expectResult("v1.0.1-wasm", "v1.0.0-wasm", true, true, "")
			expectResult("v1.0.0-wasm-rc1", "v1.0.0-rc1", false, false, "")
			expectResult("v1.0.0-rc1", "v1.0.0-wasm-rc1", false, false, "")
			expectResult("v1.0.0-foo-bar2", "v1.0.1-foo-bar1", false, true, "")
			expectResult("v1.0.0-foo-bar2", "v1.0.0-foo-bar1", true, true, "")
		})
	})
